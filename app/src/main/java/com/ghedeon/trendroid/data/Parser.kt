...
class Parser @Inject constructor() {
	...
	fun parse(html: String): List<TrendingRepoDomain> = Jsoup.parse(html)
		...
		.map { repoHtml ->
			with(repoHtml) {
				...
				val starsToday = select("span.float-sm-right").firstOrNull()?.text()
				TrendingRepoDomain(name, url, description, stars, forks, starsToday)
			}
